...
def render_tab2(username):
    with gr.TabItem("🚀 Model Evaluation"):
            ...
            with gr.TabItem("Gemini"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        ev_btn = gr.Button("🚀 Run Inference", variant="primary")
        ...
        # Wiring Events (Tab 2)
        ev_refresh.click(utils.refresh_video_list, outputs=[ev_video])
        ...
        def validate_and_run(u, t, m, v, res, think, *prompts):
            """
            Checks if at least one prompt textbox is filled before calling the backend.
            Empty fields are replaced with "None".
            """
            field_names = [
            ...
            for name, content in zip(field_names, prompts):
                if not content or not str(content).strip():
                    ...
                    processed_prompts.append("None")
                else:
                    filled_count += 1
                    processed_prompts.append(content)

            if filled_count == 0:
                error_msg = "⛔ Validation Error: All prompt sections are empty. Please fill at least one section."
                gr.Warning("All prompt sections are empty!")
                yield error_msg
                return

            if missing_fields:
                warning_msg = f"⚠️ Warning: The following {len(missing_fields)} sections are empty and will be treated as 'None':\n"
                for f in missing_fields:
                    warning_msg += f"- {f}\n"
                gr.Warning(
                    f"Missing {len(missing_fields)} prompt sections, proceeding with 'None'."
                )
                yield warning_msg + "\n🚀 Starting Inference...\n\n"

            yield from evaluation.run_gemini_inference(
                u, t, m, v, processed_prompts, res, think
            )
