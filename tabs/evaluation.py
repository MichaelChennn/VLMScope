...
def run_gemini_inference(username, task_name, model_name, video_name, prompt_list, resolution=None, thinking_level=None):
    """
    Real Gemini Inference Pipeline.
    1. Upload Video -> 2. Wait Process -> 3. Generate -> 4. Save
    """
    # --- 1. Basic Validation ---
    if not username: return "❌ Error: Not logged in."
    if not video_name: return "❌ Error: No video selected."
    ...
    # Defaults
    if not task_name: task_name = "default_task"
    
    # --- 2. Prepare Save Paths ---
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # Folder: model_results / Username / Model_Task_Timestamp
    session_id = f"{model_name.replace(' ', '')}_{task_name.replace(' ', '')}_{timestamp}"
    ...
    try:
        ...
        # Using Gemini3
        if model_name.lower().startswith("gemini-3") and (resolution and thinking_level):
            status_log.append(f"Runing Gemini-3 with Resolution: {resolution}, Thinking Level: {thinking_level}")
            yield get_log_str()
            ...
            status_log.append("Running Gemini-2.5 (or default) without special configs.")
            yield get_log_str()
        ...
        meta_data = {
            ...
            "resolution_setting": resolution if resolution else "Default for Gemini-2.5",
            "thinking_level": thinking_level if thinking_level else "Default for Gemini-2.5",
            "prompt": final_prompt,
            "timestamp": timestamp,
            "gemini_file_uri": video_file.uri,
            "token_usage": token_usage,
            "estimated_cost_usd": utils.calculate_cost(model_name, token_usage["input_tokens"], token_usage["output_tokens"] + token_usage["thinking_tokens"])
        }
        with open(meta_path, "w", encoding="utf-8") as f:
            json.dump(meta_data, f, indent=4, ensure_ascii=False)
            
        status_log.append(f"💾 Saved to: {pred_path}")
        status_log.append(f"📊 Token Usage: Input={token_usage['input_tokens']}, Thinking={token_usage['thinking_tokens']}, Output={token_usage['output_tokens']}, Total={token_usage['total_tokens']}")
        yield get_log_str()
        ...
        status_log.append(f"❌ Error:\n{str(e)}")
        yield get_log_str()

    

def run_gemini_inference_mock(username, task_name, model_name, video_name, fps, resolution, prompt):
    """
    Simulates Inference.
    ...
    # Defaults
    if not task_name: task_name = "task"
    if not model_name: model_name = "Gemini"
