...
def cut_video(video_name, start_sec, end_sec, segment_id):
    ...
    try:
        ...
        cmd = [
            ffmpeg_exe, "-y", "-ss", str(start_sec), "-i", video_path, "-t", str(duration),
...
def load_data(video_name, input_mode, raw_json, selected_file):
            ...
            if os.path.exists(meta_path):
                try:
                    with open(meta_path, 'r', encoding='utf-8') as mf:
                        ...
                        prompt_text = meta_data.get("prompt", "Prompt field missing in metadata.")
            
                except Exception as e:
                    prompt_text = f"❌ Error reading metadata: {e}"
        else:
    ...
    return gr.update(choices=choices, value=choices[0] if choices else None), parsed_data, f"✅ Loaded {len(data)} items.", prompt_text
...
def update_ui(choice, all_data, video_name):
    ...
    st = "⚪ Unlabeled"
    if hl is True: st = "✅ TRUE"
    elif hl is False: st = "❌ FALSE"
    elif hl == "not_sure": st = "⚠️ NOT SURE"
...
def handle_label(choice, all_data, comment, val):
    ...
    try: idx = int(choice.split(".")[0]) - 1
    except: return all_data, "Error", choice

    all_data[idx]['human_label'] = val
    all_data[idx]['human_comment'] = comment
    
    msg_map = {True: "Saved: TRUE", False: "Saved: FALSE", "not_sure": "Saved: NOT SURE"}
...
def save_file(data, current_user, video_name, source_rel_path):
    ...
    if source_rel_path:
        ...
        target_folder = f"{current_user}/Manual_Upload_{datetime.now().strftime('%Y%m%d')}"
    ...
    final_output = {
        "summary_statistics": {
            "true": tc, "false": fc, "not_sure": nc, "total": len(data),
...
def save_comment(choice, all_data, txt):
    ...
    try: idx = int(choice.split(".")[0]) - 1
    except: return all_data
    if idx >= 0: all_data[idx]['human_comment'] = txt
    return all_data